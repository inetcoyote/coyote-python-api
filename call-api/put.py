...
user_id = 1  # ID пользователя, которого нужно обновить
...
# Заголовки (указываем, что отправляем JSON)
headers = {
...
try:
    ...
    response = requests.put(url, json=updated_data, headers=headers, timeout=1000, verify=False)
    ...
    if response.status_code == 200:
        ...
        error = response.json()
        print(f"❌ Ошибка ввода: {error.get('message', '')}")
    else:
